...
export class EmployeeListComponent implements OnInit {
  ...
  constructor(
    ...
    this.searchEmployee.valueChanges.subscribe(res => {
      let dataFilter;
      const initData = JSON.parse(localStorage.getItem('AppComponent'));
  ...
  ngOnInit(): void {
    this.initDataEmployee = JSON.parse(localStorage.getItem('AppComponent'));
  ...
  delete(item) {
    const dialogRef = this.dialog.open(ModalConfirmationComponent, {
      ...
      data: {
        data: item,
        message: `Delete Name <br/> <b>${item.first_name} ${item.last_name}</b> ?`
      }
    });
      ...
      if (result) {
        this.initDataEmployee.forEach((element, index) => {
          if (element.id == item.id) {
            ...
            this.dataSource = this.paginator(this.initDataEmployee, this.page, this.paginatorSize);
            this.dataUser = this.dataSource.data.filter(data => { if (!data.status) { return true } });
            localStorage.setItem('AppComponent', JSON.stringify(this.initDataEmployee));
          }
        });
      }
    });
